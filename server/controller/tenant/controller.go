...
import (
	...
	"github.com/vanus-labs/vanus/server/controller/tenant/convert"
...
func (ctrl *controller) CreateToken(ctx context.Context,
	...
	random, err := primitive.RandomString(tokenRandLength)
	if err != nil {
		return nil, err
	}
...
func (ctrl *controller) CreateNamespace(ctx context.Context,
	...
	if _ns != nil {
		return nil, errors.ErrResourceAlreadyExist.WithMessage(
			fmt.Sprintf("namespace %s exist", ns.Name))
	}
