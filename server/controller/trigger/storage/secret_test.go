...
func TestSecretStorage(t *testing.T) {
	Convey("test secret", t, func() {
		...
		Convey("test credential type AK/SK", func() {
			subID := snowflake.NewTestID()
			Convey("test read", func() {
				a, _ := aes.Encrypt("test_access_key_id", secret.cipherKey)
				s, _ := aes.Encrypt("test_secret_access_key", secret.cipherKey)
		...
		Convey("test credential type gcloud", func() {
			subID := snowflake.NewTestID()
			Convey("test read", func() {
				a, _ := aes.Encrypt("{\"type\":\"service_account\"}", secret.cipherKey)
		...
		Convey("test credential type plain", func() {
			subID := snowflake.NewTestID()
			Convey("test read", func() {
				a, _ := aes.Encrypt("test_identifier", secret.cipherKey)
				s, _ := aes.Encrypt("test_secret", secret.cipherKey)
