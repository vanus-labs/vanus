...
func (m *manager) initDeadLetterEventbus(ctx context.Context, eventbusID vanus.ID) error {
	...
	deadLetterEventbusID, deadLetterEventlogID, err :=
		m.getSystemEventbusAndEventlog(ctx, deadLetterEventbusName)
	if err != nil {
		return err
	}
...
func (m *manager) initRetryEventbus(ctx context.Context) error {
	retryEventbusID, retryEventlogID, err :=
		m.getSystemEventbusAndEventlog(ctx, primitive.RetryEventbusName)
	if err != nil {
		return err
	}
