...
func (m *manager) Start() {
	go primitive.UntilWithContext(m.ctx, m.check, m.config.CheckInterval)
}
...
func (m *manager) runningTriggerWorkerHandler(ctx context.Context, tWorker TriggerWorker) {
	...
	if d > m.config.HeartbeatTimeout {
		...
		if err != nil {
			...
			log.Info(ctx).Str(log.KeyTriggerWorkerAddr, tWorker.GetAddr()).Msg(
				"running trigger worker heartbeat timeout")
		}
