...
func TestSegmentServerManager_AddAndRemoveServer(t *testing.T) {
	Convey("test add and remove server", t, func() {
		...
		So(err, ShouldBeNil)
		So(sync.MapLen(&ssm.segmentServerMapByIP), ShouldEqual, 3)
		So(sync.MapLen(&ssm.segmentServerMapByVolumeID), ShouldEqual, 3)
		...
		Convey("test remove server", func() {
			...
			_, exist = ssm.segmentServerMapByVolumeID.Load(ss1.VolumeID())
			So(exist, ShouldBeFalse)
			So(sync.MapLen(&ssm.segmentServerMapByIP), ShouldEqual, 2)
			So(sync.MapLen(&ssm.segmentServerMapByVolumeID), ShouldEqual, 2)
		})
...
func TestSegmentServerManager_Run(t *testing.T) {
	Convey("test Manager run and stop", t, func() {
		...
		mutex.Unlock()
		time.Sleep(200 * time.Millisecond)
		So(sync.MapLen(&ssm.segmentServerMapByVolumeID), ShouldEqual, 1)
		So(sync.MapLen(&ssm.segmentServerMapByIP), ShouldEqual, 1)
