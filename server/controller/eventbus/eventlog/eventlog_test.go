...
func TestEventlogManager_RunWithoutTask(t *testing.T) {
	Convey("test run", t, func() {
		...
		Convey("case: run without start", func() {
			...
			seg1.SegmentID = snowflake.NewTestID()
			segData1, _ := stdJson.Marshal(seg1)

			seg2 := new(struct {
				SegmentID vanus.ID `json:"segment_id"`
			})
			seg2.SegmentID = snowflake.NewTestID()
			segData2, _ := stdJson.Marshal(seg2)

			seg3 := new(struct {
				SegmentID vanus.ID `json:"segment_id"`
			})
			seg3.SegmentID = snowflake.NewTestID()
			...
			segment1 := createTestSegment(snowflake.NewTestID())
			segment1.ID = seg1.SegmentID
			_data1, _ := stdJson.Marshal(segment1)

			segment2 := createTestSegment(snowflake.NewTestID())
			segment2.ID = seg2.SegmentID
			_data2, _ := stdJson.Marshal(segment2)

			segment3 := createTestSegment(snowflake.NewTestID())
			...
			So(err, ShouldBeNil)
			So(sync.MapLen(&utMgr.eventlogMap), ShouldEqual, 3)
			...
			So(sync.MapLen(&utMgr.globalBlockMap), ShouldEqual, 9)
		})
	})
}
...
func TestEventlogManager_ScaleSegmentTask(t *testing.T) {
	Convey("case: run with start", t, func() {
		...
		So(sync.MapLen(&utMgr.eventlogMap), ShouldEqual, 1)
		So(sync.MapLen(&utMgr.globalSegmentMap), ShouldEqual, defaultAppendableSegmentNumber)
		So(sync.MapLen(&utMgr.globalBlockMap), ShouldEqual, defaultAppendableSegmentNumber*3)

		el2, err := newEventlog(ctx, md2, kvCli, false)
		So(err, ShouldBeNil)
		So(el2.size(), ShouldEqual, 0)
		utMgr.eventlogMap.Store(el2.md.ID.Key(), el2)
		So(sync.MapLen(&utMgr.eventlogMap), ShouldEqual, 2)
		time.Sleep(50 * time.Millisecond)
		So(el.size(), ShouldEqual, defaultAppendableSegmentNumber)
		So(el2.size(), ShouldEqual, defaultAppendableSegmentNumber)
		So(sync.MapLen(&utMgr.globalSegmentMap), ShouldEqual, defaultAppendableSegmentNumber*2)
		So(sync.MapLen(&utMgr.globalBlockMap), ShouldEqual, defaultAppendableSegmentNumber*3*2)
		...
		So(el2.size(), ShouldEqual, defaultAppendableSegmentNumber)
		So(sync.MapLen(&utMgr.globalSegmentMap), ShouldEqual, defaultAppendableSegmentNumber*2+1)
		So(sync.MapLen(&utMgr.globalBlockMap), ShouldEqual, (defaultAppendableSegmentNumber*2+1)*3)
...
func TestEventlogManager_CleanSegmentTask(t *testing.T) {
	Convey("case: run with start", t, func() {
		...
		So(el1.size(), ShouldEqual, defaultAppendableSegmentNumber)
		So(el2.size(), ShouldEqual, defaultAppendableSegmentNumber)
		So(sync.MapLen(&utMgr.segmentNeedBeClean), ShouldEqual, 0)
		So(sync.MapLen(&utMgr.globalSegmentMap), ShouldEqual, defaultAppendableSegmentNumber*2)
		So(sync.MapLen(&utMgr.globalBlockMap), ShouldEqual, defaultAppendableSegmentNumber*3*2)
	})
}
...
func TestEventlogManager_DeleteEventlog(t *testing.T) {
	Convey("test DeleteEventlog", t, func() {
		...
		Convey("test deleting", func() {
			...
			utMgr.DeleteEventlog(ctx, snowflake.NewTestID())
			...
			So(sync.MapLen(&utMgr.segmentNeedBeClean), ShouldEqual, 4)
		})
		...
		Convey("make sure delete meta in kv", func() {
			...
			_, exist := mgr.eventlogMap.Load(md.ID.Key())
			So(exist, ShouldBeFalse)
			So(sync.MapLen(&mgr.segmentNeedBeClean), ShouldEqual, 0)
		})
	})
}
...
func TestEventlogManager_UpdateSegmentReplicas(t *testing.T) {
	Convey("test UpdateSegmentReplicas", t, func() {
		...
		err = utMgr.UpdateSegmentReplicas(ctx, snowflake.NewTestID(), 3)
...
func Test_ExpiredSegmentDeleting(t *testing.T) {
	Convey("test expired segment deleting", t, func() {
		...
		Convey("test clean expired segment", func() {
			...
			So(sync.MapLen(&utMgr.segmentNeedBeClean), ShouldEqual, 3)
		})
...
func TestEventlog_All(t *testing.T) {
	Convey("test eventlog operation", t, func() {
		...
		seg1 := createTestSegment(snowflake.NewTestID())
		seg2 := createTestSegment(snowflake.NewTestID())
		seg3 := createTestSegment(snowflake.NewTestID())
		seg4 := createTestSegment(snowflake.NewTestID())
