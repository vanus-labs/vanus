...
func TestController_CreateEventbus(t *testing.T) {
	Convey("test create eventbus", t, func() {
		...
		defaultNS := snowflake.NewTestID()
		systemNS := snowflake.NewTestID()
		...
		Convey("test create a eventbus but exist", func() {
			...
			So(ok, ShouldBeTrue)
			So(et.Code, ShouldEqual, errors.ErrorCodeResourceExist)
...
func TestController_DeleteEventbus(t *testing.T) {
	Convey("test delete a eventbus ", t, func() {
		...
		Convey("deleting a doesn't exist eventbus", func() {
			res, err := ctrl.DeleteEventbus(ctx, &wrapperspb.UInt64Value{Value: snowflake.NewTestID().Uint64()})
			So(err, ShouldNotBeNil)
			So(res, ShouldBeNil)
			et, ok := err.(*errors.ErrorType)
			So(ok, ShouldBeTrue)
		...
		Convey("deleting an existed eventbus, but kv error", func() {
			...
			res, err := ctrl.DeleteEventbus(stdCtx.Background(), &wrapperspb.UInt64Value{
				Value: md.ID.Uint64(),
			})
			So(err, ShouldNotBeNil)
			So(res, ShouldBeNil)
			et, ok := err.(*errors.ErrorType)
			So(ok, ShouldBeTrue)
			So(et.Code, ShouldEqual, errors.ErrorCodeInternal)
		...
		Convey("deleting an existed eventbus success", func() {
			...
			ctrl.eventbusMap[md.ID] = md
			_, err := ctrl.DeleteEventbus(stdCtx.Background(), &wrapperspb.UInt64Value{
				Value: md.ID.Uint64(),
			})
			So(err, ShouldBeNil)
