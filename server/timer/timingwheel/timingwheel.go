...
func (tw *timingWheel) IsDeployed(ctx context.Context) bool {
	var err error
	if _, err = tw.ctrl.EventbusService().GetSystemEventbusByName(ctx,
		tw.receivingStation.eventbus); err != nil {
		return false
	}
	if _, err = tw.ctrl.EventbusService().GetSystemEventbusByName(ctx,
		tw.distributionStation.eventbus); err != nil {
		return false
	}
	return true
}
						...
						wait.Until(func() {
							...
							if tw.Push(ctx, e) {
								...
								log.Warn(ctx).
									...
									Str("delivery_time", newTimingMsg(ctx,
										e).getExpiration().Format(time.RFC3339Nano)).
...
func (tw *timingWheel) deliver(ctx context.Context, e *ce.Event) error {
	...
	if !exist {
		v, _ = tw.cache.LoadOrStore(eventbusID, tw.client.Eventbus(ctx,
			api.WithID(eventbusID.Uint64())).Writer())
	}
