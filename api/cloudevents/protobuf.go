...
func ToProto(e *event.Event) (*CloudEvent, error) {
	container := &CloudEvent{
		Id:          e.ID(),
		Source:      e.Source(),
		SpecVersion: e.SpecVersion(),
		Type:        e.Type(),
		Attributes:  make(map[string]*CloudEvent_CloudEventAttributeValue),
	}
	...
	container.Data = &CloudEvent_BinaryData{
		BinaryData: e.Data(),
	}
	if e.DataContentType() == ContentTypeProtobuf {
		...
		container.Data = &CloudEvent_ProtoData{
			ProtoData: anymsg,
		}
	}
...
func attributeFor(v interface{}) (*CloudEvent_CloudEventAttributeValue, error) {
	...
	switch vt := vv.(type) {
	case bool:
		attr.Attr = &CloudEvent_CloudEventAttributeValue_CeBoolean{
			CeBoolean: vt,
		}
	case int32:
		attr.Attr = &CloudEvent_CloudEventAttributeValue_CeInteger{
			CeInteger: vt,
		}
	case string:
		attr.Attr = &CloudEvent_CloudEventAttributeValue_CeString{
			CeString: vt,
		}
	case []byte:
		attr.Attr = &CloudEvent_CloudEventAttributeValue_CeBytes{
			CeBytes: vt,
		}
	case types.URI:
		attr.Attr = &CloudEvent_CloudEventAttributeValue_CeUri{
			CeUri: vt.String(),
		}
	case types.URIRef:
		attr.Attr = &CloudEvent_CloudEventAttributeValue_CeUriRef{
			CeUriRef: vt.String(),
		}
	case types.Timestamp:
		attr.Attr = &CloudEvent_CloudEventAttributeValue_CeTimestamp{
...
func FromProto(container *CloudEvent) (*event.Event, error) {
	...
	if container.Attributes != nil {
		...
		if attr != nil {
			if stattr, ok := attr.Attr.(*CloudEvent_CloudEventAttributeValue_CeString); ok {
