...
var (
	...
	ErrServerNotStart       = rpcerr.New("controller not start").WithGRPCCode(rpcerr.ErrorCode_SERVICE_NOT_RUNNING)
	ErrInternal             = rpcerr.New("internal error").WithGRPCCode(rpcerr.ErrorCode_INTERNAL)

	ErrFindTriggerWorkerTimeout = errors.New("find trigger worker timeout")
